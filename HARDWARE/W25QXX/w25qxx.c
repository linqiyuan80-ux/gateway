...
{	
  GPIO_InitTypeDef GPIO_InitStructure;
	...
	//DMA1_Spi2_Init();
  if(W25QXX_TYPE == 0x5e17) printf("芯片型号ZB25VQ128ASIG\r\n\r\n");
...
void sendicon(u16 x,u16 y,u16 width,u16 height,u32 ad,u16 loop,u16 count,u8 *tempsys)
{
	...
	if(Olddir==0)
	{
		LCD_Display_Dir(1);//因为图片取模是横屏取模的，如果当前是竖屏模式，则需要调转成横屏  切横竖坐标要切换
		Swap=width;
		width=height;
		height=Swap;
	}
	...
	LCD_Set_Window(0,0,lcddev.width,lcddev.height);//释放窗口
	if((Olddir==0)&&(lcddev.dir==1))LCD_Display_Dir(0);//如果之前设置是竖屏，为了显示图片强制转成横屏的，则需要还原会竖屏
